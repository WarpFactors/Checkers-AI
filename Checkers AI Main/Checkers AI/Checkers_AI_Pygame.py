...
class Game:
	...
	def event_loop(self):
		...
		"""
		self.mouse_pos = self.graphics.board_coords(pygame.mouse.get_pos()[0],pygame.mouse.get_pos()[1]) # what square is the mouse in?
			...
			if event.type == MOUSEBUTTONDOWN:
				if self.hop == False:
					if self.board.location(self.mouse_pos[0],self.mouse_pos[1]).occupant != None and self.board.location(self.mouse_pos[0],self.mouse_pos[1]).occupant.color == self.turn:
						self.selected_piece = (self.mouse_pos[0],self.mouse_pos[1])
...
class Graphics:
	def __init__(self):
		...
		self.background = pygame.image.load('Checkers Board Picture.png')
		...
		for x in range(8):
			for y in range(8):
				if board.matrix[x][y].occupant != None:
					pygame.draw.circle(self.screen, board.matrix[x][y].occupant.color, self.pixel_coords((int(x), int(y))), int(self.piece_size)) 
